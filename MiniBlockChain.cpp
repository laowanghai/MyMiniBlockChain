...
void CMiniBlockChain::getLastBlockStr(char **bs)
{
	...
	*/
	...
	/*if (pTemp->transactions == NULL)
		sprintf(bs, "{ \"index\": %d, \"previous_hash\": \"%s\", \"proof\": %d, \"timestamp\": %s, \"transactions\":[%s]}", pTemp->index, pTemp->previous_hash, pTemp->proof, pTemp->timeStamp, "");
	else
		sprintf(bs, "{ \"index\": %d, \"previous_hash\": \"%s\", \"proof\": %d, \"timestamp\": %s, \"transactions\":[%s]}", pTemp->index, pTemp->previous_hash, pTemp->proof, pTemp->timeStamp, pTemp->transactions);
	*/
	return;
}
